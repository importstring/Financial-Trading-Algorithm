...
class Agent:
    ...
    def buy(self):
        ...
        print('Action inputs:', self.action_inputs)
    ...
    def pick_tickers(self):
        string = f"""        
            ...
            Use insight and research after careful consideration and thought as they cost 100000 more than reason and 100000000 more than stockdata.
            Previous plan (for reference): {self.plan}
        ...
        while True:
            for action_name, action_func in actions:
                if action_name == 'reason':
                    ...
                    # Check if the agent decides to stop narrowing
                    if "STOP NARROWING" in refined_output.upper():
                        if 30 <= len(selected_stocks) <= 120:
                            self.active_tickers = list(selected_stocks)
                        else:
                            continue  # If not within range, continue narrowing
                    
                    # Update selected stocks based on agent's reasoning
            ...
            # Check if the number of selected stocks is within the desired range
            if 30 <= len(selected_stocks) <= 120:
                # Ask the agent if this is an optimal set
                confirmation = self.ollama.reason(f"We have narrowed down to {len(selected_stocks)} stocks. Is this an optimal set of high-potential stocks for our 20-year horizon, or should we continue narrowing? Provide your reasoning.")
                if "OPTIMAL SET" in confirmation.upper():
                    selected_stocks = self.extract_selected_stocks(confirmation)
                    self.active_tickers = list(selected_stocks)
